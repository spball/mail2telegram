...
export async function renderEmailSummaryMode(mail: EmailCache, env: Environment): Promise<EmailDetailParams> {
    ...
    const prompt = `Summarize the following email text in approximately 50 words with ${SUMMARY_TARGET_LANG}. Directly output the summarized content. If applicable, please include verification codes or links in the output.\n\n${mail.text}`;
